...
//   participants : liste des participants à la session, avec leur réponse
//   public : un booléen mis à vrai pour une session publique, faux pour une privée

Dosage = new Meteor.Collection('sessions_dose');

setPublic = function (sessionId) {
  Dosage.update(sessionId, {$set: {'open': true}});
}

setPrivate = function (sessionId) {
  Dosage.update(sessionId, {$set: {'open': false}});
}
...
Dosage.allow({
  ...
  update: function (userId, session, fields, modifier) {
    if (Meteor.user().username !== session.owner)
  ...
  remove: function (userId, session) {
    return session.owner === Meteor.user().username;
  }
});
...
Meteor.methods({
  ...
  participeSession: function (sessionId, rsvp) {
    ...
    var rsvpIndex = _.indexOf(_.pluck(session.participants, 'user'), Meteor.user().username);
    if (rsvpIndex !== -1) {
      // update existing rsvp entry
      if (Meteor.isServer) {
        // update the appropriate rsvp entry with $
        Dosage.update(
          {_id: sessionId, "participants.user": Meteor.user().username},
      ...
      Dosage.update(sessionId, {$push: {participants: {user: Meteor.user().username, rsvp: rsvp}}});
    }
  }
});
